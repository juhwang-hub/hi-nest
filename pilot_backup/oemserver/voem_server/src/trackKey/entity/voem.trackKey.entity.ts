...
export class RootOEMInfo {
    ...
    // root_oem_url : string;   // Root OEM Url

    @Column({length : 128, nullable : false})
    root_oem_url : string;   // Root OEM Url
}
  
@Entity('root_oem_server_info_table')
export class RootOEMServerInfo {
    @PrimaryColumn({length : 128, nullable : false})
    root_oem_id : string;    // Root OEM Id
...
@Entity('digital_key_info_table')
export class DigitalKeyInfo {
    @PrimaryColumn({length : 128, nullable : false})
    key_id : string // key id

    //추가한거 : key_status, device_oem
    @Column()
    key_status : number
    ...
    @Column()
    owner_device_key : string // shared key 인 경우에만 사용, 자신의 Owner 키 id 등록
    // 
