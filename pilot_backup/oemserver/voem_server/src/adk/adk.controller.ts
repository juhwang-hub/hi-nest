...
export class AdkController {  
	...
	)
	{
		...
		// 	console.log(checkList);
			...
			// let verifier = service.genVerifier(password, salt, 32768, 8, 1);  
			// let JsonVerify = async () => {
			// 	let resultString = await new CryptoService().genVerifier(password, salt, 32768, 8, 1);
			// 	let resultJSON = JSON.parse(resultString);  
			// 	let verifier = 'w0   : ' + resultJSON.w0 + '\nL   : ' + resultJSON.L + '\nSalt : ' + resultJSON.salt.data.toString('ascii') + '\n';
			// 	fs.writeFileSync(voemUserId + '.txt', verifier);
			// 	return verifier;
			// }
			// JsonVerify();
			// let resultJSON = JSON.parse(resultString);  
			// let verifier = 'w0   : ' + resultJSON.w0 + '\nL   : ' + resultJSON.L + '\nSalt : ' + resultJSON.salt;
